...
int bbs_blind_sign_with_nym(
    ...
    const bbs_ciphersuite* s = cipher_suite;
    ...
    for (uint64_t i = 0; i < num_messages; i++) {
        ...
        hash_to_scalar(s, &tmp, s->map_dst, s->map_dst_len, 1, messages[i], messages_lens[i]);
    ...
    hash_to_scalar_init(cipher_suite, &h_ctx);
	hash_to_scalar_update(cipher_suite, &h_ctx, sk, BBS_SK_LEN);

    // add B to e calculation
    ep_write_bbs(buffer, &B);
    hash_to_scalar_update(cipher_suite, &h_ctx, buffer, BBS_G1_ELEM_LEN);
	...
	// serialize (A,e)
	ep_write_bbs(signature, &B);
	bn_write_bbs(signature + BBS_G1_ELEM_LEN, &tmp);
...
int bbs_blind_verify_with_nym(
    ...
    // init domain calculation with pk and Q_1
    calculate_domain_init(s, &h_ctx, pk, num_messages + 1 + num_commited_messages + num_pseudonyms); // generators + Q_2 + blind_generators
    calculate_domain_update(s, &h_ctx, &Q_1);
    ...
    for (uint64_t i = 0; i < num_commited_messages; i++) {
        // get next generator and convert commited message to scalar
	    create_generator_next(s, generator_ctx, &H_i, (uint8_t*)BBS_BLIND_API_ID_PREFIX, 6);
        hash_to_scalar(s, &tmp, s->map_dst, s->map_dst_len, 1, comitted_messages[i], comitted_messages_lens[i]);
...
// FOR DISCUSSION: one could merge messages and commited_messages and solve the rest over indices, BUT it would still require two loops because of the differences in generators
// ALSO FOR DISCUSSION: reusability for functions from existing bbs impl very low, due to them being very interwoven and doing multiple things at once, aka domain calculation
// for which the with_nym calc does way more. Also differences in the generators and these special cases, i.e. where Q_2 maps to secret_prover_blind but is always undisclosed.
// Also under the aspect that a lot of stuff in the draft(s) will change maybe it isnt a too bad idea to leave the extension stuff for blind and with_nym on its own for now.
int bbs_blind_proof_gen_with_nym_inner(
    ...
    if (disclosed_indexes_len > num_messages) return BBS_ERROR;
    if (disclosed_commitment_indexes_len > num_commited_messages) return BBS_ERROR;
    ...
    blst_p1 B, Q_1, H_i, D, Abar, Bbar, T1, T2, pseudonym, Ut, res;
    blst_scalar domain, z, poly_eval_pseudo, poly_eval_proof, tmp;
    size_t disclosed_idx = 0, prf_undisclosed_msg_index = 0;

    // init T2, pseudonym, Ut to inf
    (void)bbs_memset(&T2.z, 0, sizeof(T2.z));
    (void)bbs_memset(&pseudonym.z, 0, sizeof(pseudonym.z));
    ...
    calculate_domain_init(s, &d_ctx, pk, num_messages + 1 + num_commited_messages + num_nym_secrets);
    ...
    uint64_t be_buffer = htobe64(disclosed_indexes_len + disclosed_commitment_indexes_len);
    ...
    for (uint64_t i = 0; i < num_messages; i++) {
        int is_disclosed = 0;
        if (disclosed_idx < disclosed_indexes_len &&
            disclosed_indexes[disclosed_idx] == i) {
            is_disclosed = 1;
            disclosed_idx++;
        }
        ...
        uint8_t *proof_ptr = (uint8_t*)proof + 3 * BBS_G1_ELEM_LEN + (3 + prf_undisclosed_msg_index) * BBS_SCALAR_LEN;
        ...
        if (is_disclosed) {
            ...
            prf(s, &tmp, 1, prf_undisclosed_msg_index, prf_cookie);
            prf_undisclosed_msg_index += 1;
    ...
    bn_write_bbs((uint8_t*)proof + 3 * BBS_G1_ELEM_LEN + (3 + prf_undisclosed_msg_index) * BBS_SCALAR_LEN, &tmp);
    ...
    for (uint64_t i = 0; i < num_commited_messages; i++) {
        int is_disclosed = 0;
        if (disclosed_idx < disclosed_commitment_indexes_len &&
            disclosed_commitment_indexes[disclosed_idx] == i) {
            is_disclosed = 1;
            disclosed_idx++;
        }
    ...
    if (num_nym_secrets > 1) {
        ...
        size_t base_idx = prf_undisclosed_msg_index - num_nym_secrets;
    ...
    blst_fp u, v;
    union bbs_hash_context hc;
    cipher_suite->expand_message_init(&hc);
    cipher_suite->expand_message_update(&hc, context_id, context_id_len);
    cipher_suite->expand_message_finalize(&hc, htg1_buf, 128, s->api_id, s->api_id_len);
    ...
    blst_p1_add_or_double(&pseudonym, &pseudonym, &res);
    // TODO add to output

    // add pseudonym to challenge calculation
    ep_write_bbs(sbuf, &pseudonym);
    hash_to_scalar_update(s, &c_ctx, sbuf, BBS_G1_ELEM_LEN);
    ...
    // FOR DISCUSSION: should I check (and return?) if Ut == IdentityG1or pseudonym == IdentityG1?
    ...
    bn_write_bbs((uint8_t*)proof + 3 * BBS_G1_ELEM_LEN + 3 * BBS_SCALAR_LEN + prf_undisclosed_msg_index * BBS_SCALAR_LEN, &tmp);
	...
	for (uint64_t i = 0; i < prf_undisclosed_msg_index; i++) {
...
int bbs_blind_proof_verify_with_nym(
    ...
    // sanity checks for indices and lengths
    if (num_disclosed_messages != num_disclosed_indexes) return BBS_ERROR;
    if (num_disclosed_committed_messages != num_disclosed_committed_indexes) return BBS_ERROR;

    const size_t proof_len_floor = 3 * BBS_G1_ELEM_LEN + 4 * BBS_SCALAR_LEN;

    if (proof_len < proof_len_floor) return BBS_ERROR;
    if ((proof_len - proof_len_floor) % BBS_SCALAR_LEN != 0) return BBS_ERROR;

    size_t U = (proof_len - proof_len_floor) / BBS_SCALAR_LEN;
    if (U == 0) return BBS_ERROR;

    size_t total_no_messages = num_disclosed_indexes + num_disclosed_committed_indexes + U - 1;
    // generators for all messages, disclosed or not
    size_t L = num_signer_known_messages;
    // blind generators for all messages, disclosed or not, but excluding length_nym_vector as thats handled seperately
    size_t M = total_no_messages - L - length_nym_vector;

    uint8_t generator_ctx[48 + 8], sbuf[BBS_G1_ELEM_LEN];
    blst_p1 B, Q_1, H_i, Abar, Bbar, T1, T2, Uv, res;
    blst_scalar domain, tmp;
    size_t disclosed_idx = 0, undisclosed_msg_index = 0;
    ...
    calculate_domain_init(s, &d_ctx, pk, total_no_messages + 1);
    ...
    uint8_t *base_proof_ptr = (uint8_t*)proof + 3 * BBS_G1_ELEM_LEN + 3 * BBS_SCALAR_LEN;

    // loop over disclosed messages
    for (uint64_t i = 0; i < L; i++) {
        int is_disclosed = 0;
        if (disclosed_idx < num_disclosed_indexes && disclosed_indexes[disclosed_idx] == i) {
            is_disclosed = 1;
            disclosed_idx++;
        }
        ...
        if (is_disclosed) {
            ...
            hash_to_scalar(s, &tmp, s->map_dst, s->map_dst_len, 1, disclosed_messages[disclosed_idx-1], disclosed_messages_lens[disclosed_idx-1]);
            ...
            uint8_t* proof_ptr = base_proof_ptr + undisclosed_msg_index * BBS_SCALAR_LEN;
            bn_read_bbs(&tmp, proof_ptr);
            undisclosed_msg_index += 1;
    ...
    for (uint64_t i = 0; i < M; i++) {
        int is_disclosed = 0;
        if (disclosed_idx < num_disclosed_committed_indexes && disclosed_committed_indexes[disclosed_idx] == i) {
            is_disclosed = 1;
            disclosed_idx++;
        }
        ...
        if (is_disclosed) {
            hash_to_scalar(s, &tmp, s->map_dst, s->map_dst_len, 1, disclosed_committed_messages[disclosed_idx-1], disclosed_comitted_messages_lens[disclosed_idx-1]);
    ...
    // FOR DISCUSSION: figure out if there is any possibility in which we dont need to reload m^_ji
