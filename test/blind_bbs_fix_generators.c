...
blind_bbs_fix_generators(void)
{
	...
	for (size_t i = 0; i < vectors_blind_generators_len; i++) {
		...
		for (size_t j = 0; j < vectors_blind_generators[i].prover_result_len; j++) {
			...
			ASSERT_EQ_PTR("prover generator",
				g_buffer,
				vectors_blind_generators[i].prover_result[j],
				BBS_G1_ELEM_LEN);
		}
	}
