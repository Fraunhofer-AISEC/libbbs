...
void print_hex_str(struct json *string, FILE *out) {
	...
	for(int i=0; i<string->len; i+=2) fprintf(out, "%c0x%.2s", i?',':'{', string->string + i);
	fprintf(out, "}");
}
...
int main(int argc, char **argv) {
	...
	// Argument parsing
	if(argc != 2) { printf("Usage: %s <ciphersuite>\n", argv[0]); exit(0); }
	...
	sprintf(filename, "fixtures-%s.c", argv[1]);
	...
	// Ciphersuite
	fprintf(out, "bbs_cipher_suite_t **const fixture_cipher_suite = &%s; \n\n", cipher_suite);
	...
	tmp = json_object_get(j2, "message");
	fprintf(out, "\t{ .msg = h2s_message%d, .msg_len = %d, ", i = 0, tmp->len / 2);
	tmp = json_object_get(j2, "dst");
	fprintf(out, ".dst = h2s_dst%d, .dst_len = %d, .result = ", i++, tmp->len / 2);
	tmp = json_object_get(j2, "scalar");
	print_hex_str(tmp, out);
	fprintf(out, "},\n");
	for(struct json *k=json_object_get(j, "cases")->value; k; k = k->next) {
		tmp = json_object_get(k, "message");
		fprintf(out, "\t{ .msg = h2s_message%d, .msg_len = %d, ", i, tmp->len / 2);
		tmp = json_object_get(j, "dst");
		fprintf(out, ".dst = h2s_dst%d, .dst_len = %d, .result = ", i++, tmp->len / 2);
	...
	fprintf(out, "\t{ .key_material = keygen_material, .key_material_len = %d, ", json_object_get(j, "keyMaterial")->len);
	fprintf(out, ".key_info = keygen_info, .key_info_len = %d, ", json_object_get(j, "keyInfo")->len);
	fprintf(out, ".key_dst = keygen_dst, .key_dst_len = %d, ", json_object_get(j, "keyDst")->len);
	...
	for(filenum = 1; 1; filenum++) {
		...
		fprintf(out, ", .header = signature%d_header, .header_len = %d", filenum, tmp->len/2);
		fprintf(out, ", .num_messages = %d", json_array_len(json_object_get(j, "messages")));
	...
	for(filenum = 1; 1; filenum++) {
		...
		print_hex_str(json_object_get(j, "signature"), out);
		tmp = json_object_get(j, "header");
		fprintf(out, ", .header = proof%d_header, .header_len = %d", filenum, tmp->len/2);
		tmp = json_object_get(j, "presentationHeader");
		fprintf(out, ", .presentation_header = proof%d_header, .presentation_header_len = %d", filenum, tmp->len/2);
		fprintf(out, ", .num_messages = %d", json_array_len(json_object_get(j, "messages")));
		...
		fprintf(out, ", .disclosed_indexes_len = %d", json_array_len(json_object_get(j, "disclosedIndexes")));
		tmp = json_object_get(j, "proof");
		fprintf(out, ", .result = proof%d_proof, .result_len = %d", filenum, tmp->len/2);
		tmp = json_object_get(j, "result");
		fprintf(out, ", .result_valid = %d },\n", JSON_TRUE == json_object_get(tmp, "valid")->type);
		json_free(j);
		free(f);
